...
- **瓶頸警示**：偵測各站「出滿框（有產品）」被 AGV 卡住堆積並即時提示（退空框不計，因其無產品、作用是讓前一站繼續）；記錄峰值／累計／次數堆積時間。
...
- **瓶頸成因診斷**：依「服務該站的 AGV 群組稼動率」自動判斷成因——群組吃滿→缺車（建議該組 AGV +1）；群組有餘裕→瓶頸在該站本身節拍（建議該站機台 +1）；提供「套用 +1 並重跑」捷徑直接看前後差異。另：AGV 取滿框採「先出先拿」，等越久者越優先。
